...
def kill_php_proc():
    with open("Tracker/Settings.json", "r") as jsonFile:
    ...
    try:
        for i in pid:
            ...
            pid.clear()
            data["pid"] = []
            with open("Tracker/Settings.json", "w") as jsonFile:
...
def run_php_server(port):
    with open(f"Tracker/log/php-{md5_hash().hexdigest()}.log","w") as php_log:
        proc_info = subprocess.Popen(("php","-S",f"localhost:{port}","-t","Tracker"),stderr=php_log,stdout=php_log).pid
